...
var cursor_texture = load("res://addons/BottledTileMap/plugin_icon.png")
...
func _draw() -> void:
	var angle_offset = 0
	if mode == MODE_ISOMETRIC: angle_offset = PI/4
	...
	angle_offset = 0
	if mode == MODE_ISOMETRIC: angle_offset = s_display_length
	if s_axis > 0:
	...
	if not tm_hints.empty():
		for i in tm_hints.size():
			draw_string(font, get_local_mouse_position()+Vector2(10,15*i), tm_hints[i].name, Color.aqua)
...
#			if c == Vector2.ZERO: continue
			if p_replacing or get_cell(x+c.x,y+c.y) == -1:
				...
				if p_allow_autotile: update_bitmask_area(Vector2(x+c.x,y+c.y))
		...
		if (c_size.x > 1 or c_size.y > 1) and can_trigger:
			...
			# use spray tool
				...
				for t in spray(Vector2(x,y)):
					setcellv(t,id,fy,fx,_t,_a)
			can_trigger = true
		else: setcell(x,y,id,fy,fx,_t,_a) # case that doesn't change from a normal tilemap
		if not can_origin: can_origin = true
		return
		...
		if (c_size.x > 1 or c_size.y > 1) and can_trigger:
			...
			if c_spray_density == 0:
				for t in get_tiles_with_brush(Vector2(x,y)):
					...
					if id != NO_TILE: setcellv(t,id,fy,fx,_t,get_random_subtile(id, _a))
			else:
				for t in spray(Vector2(x,y)):
					id = get_tileID(t.x-origin.x,t.y-origin.y)
					if id != NO_TILE: setcellv(t,id,fy,fx,_t,get_random_subtile(id, _a))
	...
	# draw with whole pattern
		...
		if can_trigger:
			...
			can_trigger = true
		else: setcell(x,y,id,fy,fx,_t,get_random_subtile(id, _a))


func setcell(x:int,y:int,id,fy=false,fx=false,_t=false,_a=Vector2()):
	if r_no_draw_on == get_cell(x,y) or (r_only_draw_on != NO_TILE and get_cell(x,y) != r_only_draw_on): return
	...
	if p_allow_autotile: update_bitmask_area(Vector2(x,y))
...
func draw_symmetry(x:int,y:int,id,fy=false,fx=false,_t=false,_a=Vector2()):
	...
	if s_axis == X_AXIS:
		...
		if p_allow_autotile: update_bitmask_area(Vector2(x,y-2*(y-s_position.y)-1))
	elif s_axis == Y_AXIS:
		.set_cell(x-2*(x-s_position.x)-1,y,id,fy,fx,_t,_a)
		if p_allow_autotile: update_bitmask_area(Vector2(x-2*(x-s_position.x)-1,y))
	elif s_axis == BOTH_AXIS:
		.set_cell(x-2*(x-s_position.x)-1,y,id,fy,fx,_t,_a)
		.set_cell(x,y-2*(y-s_position.y)-1,id,fy,fx,_t,_a)
		.set_cell(x-2*(x-s_position.x)-1,y-2*(y-s_position.y)-1,id,fy,fx,_t,_a)
		if p_allow_autotile:
			update_bitmask_area(Vector2(x,y-2*(y-s_position.y)-1))
			update_bitmask_area(Vector2(x-2*(x-s_position.x)-1,y))
			update_bitmask_area(Vector2(x-2*(x-s_position.x)-1,y-2*(y-s_position.y)-1))
...
func get_origin(x,y):
	...
	if p_center == CENTER.origin: origin = Vector2(0,0)
	elif p_center == CENTER.mouse: origin = Vector2(x,y)
...
func set_global_replacing(value, from=r_replace_x_by_y.x, to=r_replace_x_by_y.y):
	if not value: return
	var to_replace = []
	if from == ALL_TILES: to_replace = get_used_cells() # replace all tiles
	else: to_replace = get_used_cells_by_id(from) # replace tiles with corresponding id
...
func read_used_tilemaps(value):
	...
	if t_tilemap_list.empty(): return
	used_tilemaps.clear()
	curr_tilemap = null
...
func read_used_cursors(value):
	c_use = value
	if c_multi_cursor_list.empty(): return
	used_cursors.clear()
...
func set_curr_tilemap(value):
	...
	for tile in curr_tilemap.get_used_cells():
		.set_cellv(tile, curr_tilemap.get_cellv(tile), curr_tilemap.is_cell_x_flipped(tile.x, tile.y),\
