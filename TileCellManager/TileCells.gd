...
var a = [] #0: 1: 2:tilemap 3:tile 4:tileset
var tileset:TileSet
var tilemap:TileMap
var id:int
...
func _on_LineEdit_text_changed(new_text):
	if a.size() == 0 or tileset.tile_get_name(id) == new_text: return
	tileset.tile_set_name(id, new_text)

func _on_SpinBox_value_changed(value):
	if a.size() == 0 or z_index.value == tileset.tile_get_z_index(id): return
	tileset.tile_set_z_index(id, value)

func _on_ColorPickerButton_color_changed(color):
	if a.size() == 0 or Modulation.color == tileset.tile_get_modulate(id): return
	tileset.tile_set_modulate(id, color)

func _on_Shape_text_changed(new_text) -> void:
	if a.size() == 0: return
	tileset.tile_set_shape_offset(id,0,Vector2(float(shape.get_node("x").text),float(shape.get_node("y").text)))

func _on_Tex_text_changed(new_text) -> void:
	if a.size() == 0: return
	tileset.tile_set_texture_offset(id, Vector2(float(tex.get_node("x").text),float(tex.get_node("y").text)))
...
func _on_SaveGroups_pressed() -> void:
	...
	var g_list = []; var list:Array;
	
	for g in dict_g.keys():
		if not g in tile_groups and id in dict_g[g]: # delete if needed (by group)
			dict_g[g].erase(id)
			if dict_g[g].empty(): dict_g.erase(g)
	for g in tile_groups:
		if not dict_g.has(g): dict_g[g] = []
		if not id in dict_g[g]: dict_g[g].append(id) # by group
		if not g in g_list: g_list.append(g) # by id
	dict_id[id] = g_list
